...
object Main extends App {
  ...
  def map(str: String): Option[(String, String)] = {
    Try {
      ...
      (id, Utils.stringPairAsText(domain, rawHour.toString))
    }.toOption
  ...
  def reduce(seq: Seq[String]): Option[String] = {
    ...
    Try {
      val listOfDomains = seq.map(Utils.textAsStringPair)
  ...
  def sortFunc(str: String) = {
    val seqAsString = str.drop(1).dropRight(1)
    -seqAsString.split(",").length
  }

  val result = sc.textFile(dataLocation).flatMap(map).groupBy(_._1).mapValues(_.map(_._2).toList).mapValues(reduce).filter(_._2.nonEmpty).mapValues(_.get).sortBy(v => sortFunc(v._2)).take(100)
